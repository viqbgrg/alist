...
func (p *Thunder) Name() string {
	return "thunder"
}

func (p *Thunder) Items() []model.SettingItem {
	return nil
}

func (p *Thunder) Run(task *tool.DownloadTask) error {
	return errs.NotSupport
}

func (p *Thunder) Init() (string, error) {
	p.refreshTaskCache = false
	return "ok", nil
}

func (p *Thunder) IsReady() bool {
	return true
}

func (p *Thunder) AddURL(args *tool.AddUrlArgs) (string, error) {
	// 添加新任务刷新缓存
	...
	t, err := thunderDriver.OfflineDownload(ctx, args.Url, parentDir, "")
	if err != nil {
...
func (p *Thunder) Remove(task *tool.DownloadTask) error {
...
func (p *Thunder) Status(task *tool.DownloadTask) (*tool.Status, error) {
	storage, _, err := op.GetStorageAndActualPath(task.DstDirPath)
	if err != nil {
		return nil, err
	}
	...
	tasks, err := p.GetTasks(thunderDriver)
	if err != nil {
		return nil, err
	}
	s := &tool.Status{
